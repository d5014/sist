...
$(function(){
	$(".btnSendBoardLike").click(function(){
		...
		let url = "${pageContext.request.contextPath}/sbbs/insertBoardLike.do";
		let num = "${dto.num}";
...
function countReplyAnswer(answer) {
	let url = "${pageContext.request.contextPath}/sbbs/countReplyAnswer.do";
	let query = "answer=" + answer;
			...
			<div class="body-main">
					...
					<tbody>
						...
						<tr>
							<td colspan="2" class="text-center p-3">
								<button type="button" class="btn btn-outline-secondary btnSendBoardLike" title="좋아요"><i class="far fa-hand-point-up" style="color: ${isUserLike?'blue':'black'}"></i>&nbsp;&nbsp;<span id="boardLikeCount">${dto.likeCount}</span></button>
							</td>
						</tr>
						...
						<tr>
							<td colspan="2">
								...
								<c:if test="${not empty dto.saveFilename}">
									<a href="${pageContext.request.contextPath}/sbbs/download.do?num=${dto.num}">${dto.originalFilename}</a>
						...
						<tr>
							<td colspan="2">
								...
								<c:if test="${not empty preReadDto}">
									<a href="${pageContext.request.contextPath}/sbbs/article.do?${query}&num=${preReadDto.num}">${preReadDto.subject}</a>
								</c:if>
							</td>
						</tr>
						<tr>
							<td colspan="2">
								...
								<c:if test="${not empty nextReadDto}">
									<a href="${pageContext.request.contextPath}/sbbs/article.do?${query}&num=${nextReadDto.num}">${nextReadDto.subject}</a>
								</c:if>
							</td>
						</tr>
					</tbody>
				...
				<table class="table table-borderless">
					<tr>
						<td width="50%">
							<c:choose>
								<c:when test="${sessionScope.member.userId==dto.userId}">
									<button type="button" class="btn btn-light" onclick="location.href='${pageContext.request.contextPath}/sbbs/update.do?num=${dto.num}&category=${category}&page=${page}';">수정</button>
								</c:when>
								<c:otherwise>
						...
						<td class="text-end">
							<button type="button" class="btn btn-light" onclick="location.href='${pageContext.request.contextPath}/sbbs/list.do?${query}';">리스트</button>
						</td>
					</tr>
				</table>
