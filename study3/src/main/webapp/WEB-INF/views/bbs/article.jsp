...
function deleteBoard(){
	...
	if(confirm('게시글을 삭제하시겠습니까 ? ')){
		let url="${pageContext.request.contextPath}/bbs/delete.do?num=${dto.num}&${query}";
		location.href=url;
	}
}
	...
	<table class="table table-border table-article">
		...
		<tbody>
			...
			<tr>
				<td colspan="2">
					이전글 : 
					<c:if test="${not empty preReadDto}"><!-- 이전글이 있으면 -->
			...
			<tr>
				<td colspan="2">
					다음글 : 
					<c:if test="${not empty nextReadDto}"><!-- 다음글이 있으면 -->
